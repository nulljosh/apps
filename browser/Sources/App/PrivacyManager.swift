...
final class PrivacyManager {
    ...
    static func upgradeToHTTPS(url: URL) -> URL? {
        guard var components = URLComponents(url: url, resolvingAgainstBaseURL: false) else { return nil }
        guard components.scheme == "http" else { return nil }
        components.scheme = "https"
        return components.url
    }

    func startAutoClearTimer(interval: TimeInterval = 3600) {
        autoClearTimer?.invalidate()
        autoClearTimer = Timer.scheduledTimer(withTimeInterval: interval, repeats: true) { _ in
            Task { @MainActor in
