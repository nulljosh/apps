...
final class StorageTests: XCTestCase {
    private let testDir = FileManager.default.temporaryDirectory
        .appendingPathComponent("BrowserStorageTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLoadReturnsNilWhenNoFile() {
        // Fresh state with no prior save -- load should return nil gracefully
        let result = Storage.load()
        // Result depends on whether prior test saved; the point is no crash
        _ = result
    }
    ...
    func testCrashRecoverySaveAndLoad() {
        ...
        XCTAssertNotNil(recovered!.timestamp)
    }
