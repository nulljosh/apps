...
function LiveMapBackdrop({ dark, mapLayers, onMapReady }) {
  ...
  useEffect(() => {
    ...
    payload.emergencyIncidents.slice(0, 120).forEach((inc) => {
      ...
      const popup = new maplibreRef.current.Popup({ offset: 10, closeButton: false, maxWidth: '200px', className: 'monica-map-popup' })
    ...
    if (mapLayers.flights !== false) {
      payload.flights.slice(0, 120).forEach((fl) => {
        ...
        el.addEventListener('click', (e) => {
          e.stopPropagation();
          if (activePopupRef?.current) activePopupRef.current.remove();
          const popup = new maplibreRef.current.Popup({ offset: 14, closeButton: true, maxWidth: '320px', className: 'monica-map-popup' })
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <button
        ...
        style={{ position: 'absolute', right: 14, bottom: 56, zIndex: 2, width: 34, height: 34, border: '1px solid rgba(255,255,255,0.24)', borderRadius: 9999, background: 'rgba(2,6,23,0.82)', color: '#ff5a52', font: '700 15px ui-monospace,SFMono-Regular,Menlo,Consolas,monospace', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
      >
        ⌖
      </button>
