...
const TABS = ['portfolio', 'budget', 'spending', 'subscriptions', 'dashboard', 'roadmap'];
    ...
    >
      ...
      <div style={{ padding: '16px 16px 12px' }}>
        ...
        {(() => {
          ...
          if (isAuthenticated && (ageMs === null || ageMs > 60 * 60 * 1000)) {
            return <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.5px', color: '#d4a843', background: 'rgba(212,168,67,0.15)', borderRadius: 4, padding: '2px 6px', marginTop: 4, display: 'inline-block' }}>STALE</span>;
          }
          if (ageMs !== null && ageMs > 5 * 60 * 1000) {
            return <div style={{ fontSize: 11, color: '#d4a843', marginTop: 4 }}>Synced {Math.round(ageMs / 60000)}m ago</div>;
          }
          return null;
        })()}
      </div>
      ...
      <div style={{ padding: '0 16px 80px', maxWidth: 720, margin: '0 auto' }}>
        ...
        {tab === 'subscriptions' && (() => {
          ...
          const monthlyTotal = activeSubs.reduce((sum, s) => sum + (s.amount || 0), 0);
          return (
            <>
              <Card dark={dark} t={t} style={{ marginBottom: 16 }}>
                <div style={sectionStyle}>
                  ...
                  {activeSubs.map((sub) => (
                    <div key={sub.name} style={rowStyle}>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 14 }}>{sub.name}</div>
                        <div style={{ fontSize: 11, color: t.textTertiary }}>
                          {sub.provider}{sub.renewDay ? ` · renews ${sub.frequency === 'monthly' ? `the ${sub.renewDay}` : sub.frequency}` : ''}
                        </div>
                      </div>
                      <div style={{ fontWeight: 600, fontSize: 14, fontVariantNumeric: 'tabular-nums' }}>
              ...
              {inactiveSubs.length > 0 && (
                <Card dark={dark} t={t} style={{ marginBottom: 16 }}>
                  <div style={sectionStyle}>
                    <div style={labelStyle}>Inactive</div>
                    {inactiveSubs.map((sub) => (
                      <div key={sub.name} style={{ ...rowStyle, opacity: 0.5 }}>
                        <div>
                          <div style={{ fontWeight: 600, fontSize: 14 }}>{sub.name}</div>
                          <div style={{ fontSize: 11, color: t.textTertiary }}>{sub.provider}{sub.note ? ` · ${sub.note}` : ''}</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {tab === 'roadmap' && <EpiphanyFinance t={t} spending={spendingChronological} />}
      </div>
    </div>
  );
}
