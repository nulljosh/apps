...
export default function AiPanel({ open, onClose, subscription }) {
      ...
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="ai-panel-messages" ref={scrollRef}>
          ...
          {error === 'upgrade_required' ? (
            <div className="ai-error" style={{ cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setShowPricing(true)}>
              Free limit reached. Tap to upgrade to Premium.
            </div>
          ) : error ? (
            <div className="ai-error">{error}</div>
          ) : null}
          {showPricing && <PricingPage dark subscription={subscription} onClose={() => setShowPricing(false)} t={{ text: '#fff', textSecondary: '#8e8e93', textTertiary: '#636366', accent: '#0071e3' }} />}
        </div>
