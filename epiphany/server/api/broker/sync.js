...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!accounts || accounts.length === 0) {
      const linkUrl = await adapter.loginLink();
      return res.status(200).json({ ok: true, linked: false, linkUrl });
    }

    const [holdings, balance] = await Promise.all([adapter.getHoldings(), adapter.getBalance()]);
    const snapshot = { holdings, balance, syncedAt: new Date().toISOString() };
