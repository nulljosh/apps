...
import { isPro, checkFreeAiLimit } from './gates.js';
...
export default async function handler(req, res) {
  ...
  const pro = await isPro(session);
  if (!pro) {
    const limit = await checkFreeAiLimit(session);
    if (!limit.allowed) {
      return res.status(403).json({ error: 'upgrade_required', message: `Free tier limit reached (${limit.limit} AI queries/day). Upgrade to Premium for unlimited access.` });
    }
  }
