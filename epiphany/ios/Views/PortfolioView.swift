...
extension PortfolioView {
        ...
        if !debt.isEmpty {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                ForEach(Array(debt.enumerated()), id: \.element.name) { i, item in
                    debtItemRow(item, pct: totalDebt > 0 ? item.balance / totalDebt : 0, color: colors[i % colors.count])
                }
            }
            .padding(.horizontal)
        }
    ...
    @ViewBuilder
    func debtItemRow(_ item: FinanceData.DebtItem, pct: Double, color: Color) -> some View {
        HStack(spacing: 12) {
            Text(item.name)
                .font(.caption.weight(.semibold))
                .foregroundStyle(Palette.textSecondary)
...
extension PortfolioView {
    ...
    @ViewBuilder
    func subscriptionRow(_ sub: FinanceData.Subscription) -> some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                Text(sub.name)
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(.primary)
                Text(sub.provider + (sub.renewDay.map { " · renews \($0)th" } ?? ""))
                    .font(.caption)
                    .foregroundStyle(Palette.textSecondary)
            }
            Spacer()
