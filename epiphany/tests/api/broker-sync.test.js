...
describe('SnapTradeAdapter', () => {
  ...
  it('is read-only — placeOrder throws', async () => {
    const a = new SnapTradeAdapter({ clientId: 'x', consumerKey: 'y', userId: 'u', userSecret: 's' });
    await expect(a.placeOrder({ sym: 'AAPL', action: 'buy' })).rejects.toThrow(/read-only/);
  });
