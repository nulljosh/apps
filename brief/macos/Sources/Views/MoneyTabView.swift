...
struct MoneyTabView: View {
    ...
    var scenarios:        [Scenario]    { rcmp ? caseScenarios  : familyCaseScenarios }
    var activeDamageHeads: [DamageHead] { rcmp ? damageHeads    : familyDamageHeads }
                    ...
                    VStack(spacing: 0) {
                        ...
                        ForEach(activeDamageHeads) { h in
                            HStack(alignment:.top) {
                                ...
                                Text(h.note).font(.system(size:10,design:.monospaced)).foregroundStyle(.secondary).frame(width:130,alignment:.leading)
                            }
                            .padding(.vertical,6)
                            Divider()
                        }
