...
struct MacGraphCanvasView: View {
    ...
    private func drawLabels(ctx: GraphicsContext, size: CGSize) {
        ...
        while xVal < size.width {
            ...
            if abs(val) > 0.001 {
                ctx.draw(Text(formatLabel(val)).font(font).foregroundColor(labelColor), at: CGPoint(x: xVal, y: cy + 14), anchor: .top)
            }
            xVal += step
        }
        ...
        while yVal < size.height {
            ...
            if abs(val) > 0.001 {
                ctx.draw(Text(formatLabel(val)).font(font).foregroundColor(labelColor), at: CGPoint(x: cx - 8, y: yVal), anchor: .trailing)
            }
            yVal += step
        }
    }
...
extension View {
    func onScrollWheel(perform action: @escaping (NSEvent) -> Void) -> some View {
        background(ScrollWheelHandler(action: action))
    }
}
...
struct ScrollWheelHandler: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> ScrollWheelView {
        let v = ScrollWheelView()
        v.action = action
        return v
    }
    func updateNSView(_ v: ScrollWheelView, context: Context) { v.action = action }
}
